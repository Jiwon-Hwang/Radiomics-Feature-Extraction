...
void LocalIntensity::calcLocalIntensityPeak(short* pusImage, unsigned char* pucMask) {
	...
	*/
	...
	for (int row = posOfCenter.first - nrPixelsY; row < posOfCenter.first + nrPixelsY; row++) {
		for (int col = posOfCenter.second - nrPixelsX; col < posOfCenter.second + nrPixelsX; col++) {
			tempDist.first = float(row) * pixelSpacingY + pixelSpacingY / 2 - (posOfCenter.first + 0.5) * pixelSpacingY; // X축(row) 방향 중점과의 거리(mm) => X_temp - X_center
			tempDist.second = float(col) * pixelSpacingX + pixelSpacingX / 2 - (posOfCenter.second + 0.5) * pixelSpacingX; // Y
