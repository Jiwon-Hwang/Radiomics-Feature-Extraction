...
#define DEBUG_LOG_DETAIL(function, line, x) std::cerr << function << "[" << line << "]" << std::endl \
													  << x
#include <vld.h>
#else
#define DEBUG_LOG(x)
...
public:
	QThread *m_pThread;
	std::vector<std::string> m_sReadFileList;
#endif

public:
	enum READIMAGE_CLEAR_OPTION
	{
		KEEP_PREV_DATAS,
	...
	enum LOAD_STATUS
	{
		...
		UNREAD = 0,
		UNREADBLE_FILE = -1,
		UNREADBLE_FILE_FORMAT = -2,
		INPUT_PARAM_ERR = -3,
		DUPLICATE_FILE = -4,
		DCM_FILE_FORMAT_ERR = -5,
		DCM_FILE_DATASET_ERR = -6,
		DCM_FILE_DECOMPRESS_FAIL = -7,
		DCM_IMG_ERR = -8,
		EXCEPT_FILE = -9,
		BAD_ALLOC = -999
	};
...
private:
	...
	std::vector<std::string> m_sExceptExtensions;
	...
	{
		int nType;
		InputDataStruct *pParent;
		std::vector<InputDataStruct *> pChildren;
	};
	InputDataStruct *m_pInputDataStruct;
...
public:
	...
	// config
	void readConfig(std::string sConfigFilePath);
...
private:
	...
	bool saveImage_libtiff(int nSeriesIdx, int nImageIdx, const char *pcImagePath, int nSaveType = SAVE_IMAGE_ONLY, int nSaveOption = SAVE_DEFAULT); // nImageIdx?? -1??? multiple page
...
private:
	bool findTag(int nTagType, std::vector<std::string> &sResuls);
...
private:
	...
	bool isMask(std::string sPath);
	bool isExcludeFile(std::string sPath);

	// [series, image renaming rule]
